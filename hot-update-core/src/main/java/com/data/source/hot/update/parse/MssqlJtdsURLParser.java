...
public class MssqlJtdsURLParser extends AbstractURLParser {
    ...
    protected static final Logger logger = LoggerFactory.getLogger(MssqlJdbcURLParser.class);

    public MssqlJtdsURLParser(String url) {
        super(url);
    }

    public MssqlJtdsURLParser(String url, String dbType) {
        super(url);
        this.dbType = dbType;
    }
        ...
        try {
            ...
            if (hostSegment.length > 1) {
                ...
                return new ConnectionInfo(
                        dbType, sb.substring(0, sb.length() - 1), fetchDatabaseNameFromURL());
            } else {
                String[] hostAndPort = hostSegment[0].split(":");
                if (hostAndPort.length != 1) {
                    return new ConnectionInfo(
                            dbType, hostAndPort[0], Integer.valueOf(hostAndPort[1]),
            ...
            logger.error("parse mssql connection info error, url:{}", url, e);
            return new ConnectionInfo(dbType, url, "UNKNOWN");
        }
    }
}
